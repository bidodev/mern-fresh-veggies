...
const createSendToken = (user, status, req, res) => {
  ...
  const jwtCookieExpiresInDays = Number(process.env.JWT_COOKIE_EXPIRES_IN || 0);

  res.cookie('jwt', jwt, {
    expires: new Date(
      Date.now() + jwtCookieExpiresInDays * 24 * 60 * 60 * 1000
    ),
    httpOnly: true,
  ...
  const { _id, email, name, role, images, slug, config } = user;
